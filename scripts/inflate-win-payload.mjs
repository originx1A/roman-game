...
function assemble(prefix, outPath) {
  ...
  if (fs.existsSync(single)) {
    const buf = zlib.gunzipSync(Buffer.from(fs.readFileSync(single, 'utf8'), 'base64'))
    checkSha(buf, base, dir)
    fs.mkdirSync(path.dirname(outPath), { recursive: true })
    fs.writeFileSync(outPath, buf)
    console.log('wrote', outPath, buf.length, 'from single gz.b64')
    return
  }

  const gzParts = fs.readdirSync(dir)
    .filter((f) => f.startsWith(base + '.gz.b64.'))
    .sort((a, b) => Number(a.split('.').pop()) - Number(b.split('.').pop()))
  if (gzParts.length) {
    const b64 = gzParts.map((f) => fs.readFileSync(path.join(dir, f), 'utf8')).join('')
    const buf = zlib.gunzipSync(Buffer.from(b64, 'base64'))
    checkSha(buf, base, dir)
    fs.mkdirSync(path.dirname(outPath), { recursive: true })
    fs.writeFileSync(outPath, buf)
    console.log('wrote', outPath, buf.length, 'from', gzParts.length, 'gz.b64 parts')
    return
  }
